    ...
    def __init__(self):
        ...
        if not self.openrouter_api_key or self.openrouter_api_key == "YOUR_OPENROUTER_API_KEY":
            ...
            if "gemini" in self.model.lower():
                ...
                logger.warning(f"⚠️ Используется модель {self.model} (рекомендуется Gemini)")
...
ТВОЯ ОСНОВНАЯ ЗАДАЧА: Создавать контент, который будет вдохновлять на отношения по любви и семье, обучать и поддерживать аудиторию в построении гармоничных и счастливых отношений, не навязчиво переобучать аудиторию тому,что они подбирали себе партнеров по внутренним ценностям, а не по внешним признакам ил по денежному достатку партнера"""
        ...
        for day in range(3):  # Только 3 дня
            ...
            for post_type in ["утро", "день", "вечер"]:
                ...
                if post_data:
                    ...
                    # Считаем запросы: 1 для основного поста + 1 для промпта (только для дневных постов)
                    if post_type == "день":
                        api_requests_count += 2  # основной пост + 1 промпт (только английский)
                    else:
    ...
    async def _generate_single_post(self, date: str, post_type: str) -> Dict[str, Any]:
        ...
        *Важно помнить:* доверие строится не за один день, но разрушается в один момент."""
            ...
            # Генерируем промпт для Midjourney только для дневных постов (только на английском)
            midjourney_prompt_ru = ""
            midjourney_prompt_en = ""
            if time == "14:00":  # Только для дневных постов
                logger.info("🎨 Генерируем промпт Midjourney (английский) для дневного поста")
    ...
    async def _generate_midjourney_prompt(self, post_text: str, language: str) -> str:
        """Генерирует промпт для Midjourney (только английский для дневных постов)"""
        try:
            if language == "ru":
